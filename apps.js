...
function setW(m,v){
  ...
  liveSets[m].forEach(s=>{ if(s && (!s.w||s.w===0)) s.w=userWeights[m]; });
  render();
}
...
function renderExercise(x){
  const r=RULES[x.t], id=x.m.id, wt=userWeights[id]||0;
  return `
  <div class="exercise">
    <div class="weight">
...
function renderDayView(){
  const d=rotationPlans[0][selectedDay];
  workout.innerHTML=`<h1>${selectedDay}</h1>`+d.ex.map(renderExercise).join('');
}

function renderCardio(){ workout.innerHTML="<h2>Cardio</h2>"; }
function renderTrends(){ workout.innerHTML="<h2>Trends</h2>"; }
function renderWeeklySummary(){ workout.innerHTML="<h2>Weekly Summary</h2>"; }

/* ===== ROUTER ===== */
function selectDay(d){ selectedDay=d; currentView='workout'; render(); }
